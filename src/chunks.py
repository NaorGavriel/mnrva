...
from embeddings import aembed_text, embed_text
...
def _search_filter(language: str | None) -> "models.Filter | None":
    """Build the optional payload filter shared by both prefetches of a hybrid search."""
    conditions = []
...
def _sparse_prefetch(query_text: str, query_filter: "models.Filter | None") -> models.Prefetch:
    """Build the BM25 sparse-vector prefetch shared by sync and async hybrid search."""
...
def _point_to_search_result(point) -> ChunkSearchResult:
    """Map one Qdrant scored point into a `ChunkSearchResult`."""
...
def _record_to_chunk(record) -> Chunk:
    """Map one Qdrant retrieved record into a `Chunk`."""
    return Chunk(
        id=str(record.id),
...
def search_chunks(
    ...
    query_filter = _search_filter(language)
    ...
    response = client.query_points(
        ...
        prefetch=[
            ...
            _sparse_prefetch(query_text, query_filter),
        ],
...
async def asearch_chunks(
    ...
    """Async twin of search_chunks, for the query agent's async graph."""
    ...
    response = await client.query_points(
        ...
        prefetch=[
            models.Prefetch(
                query=await aembed_text(query_text), using=DENSE_VECTOR_NAME, filter=query_filter
            ),
            _sparse_prefetch(query_text, query_filter),
        ],
        query=models.FusionQuery(fusion=models.Fusion.RRF),
...
async def aget_chunks_by_id(client: AsyncQdrantClient, collection_name: str, chunk_ids: list[str]) -> list[Chunk]:
    """Async twin of get_chunks_by_id, for the query agent's async graph."""
    records = await client.retrieve(collection_name=collection_name, ids=chunk_ids, with_payload=True)
    return [_record_to_chunk(record) for record in records]
